...
> **Este é um projeto independente** do seu outro painel ("Tarefas-2.0" / Minhas Tarefas) — mesma estrutura de código, mas sem a aba de Saldo. Ele precisa do **próprio projeto Supabase**, do **próprio repositório no GitHub** e do **próprio projeto na Vercel** — não reaproveite os do outro projeto, senão os dois vão brigar pelos mesmos dados.
...
`CREDENTIALS_ENCRYPTION_KEY` protege as senhas salvas na aba "Senhas" — gere uma string aleatória longa (ex.: `openssl rand -hex 32`) e **guarde-a em lugar seguro fora do sistema**. Se ela mudar ou se perder, tudo que já foi salvo em "Senhas" fica ilegível para sempre — não tem como recuperar.
